...
class DanTranhTuner {
    ...
    detectPitch() {
        ...
        // Detect when sound ENDS (for labeling)
        if (this.isPlaying && !hasEnoughSound) {
            // Sound just stopped - add label to the last active string
            this.isPlaying = false;
            console.log(`🛑 SOUND ENDED! RMS: ${rms.toFixed(4)}`);

            // Add label to the last string that was being tracked
            if (this.lastActiveString !== undefined) {
                ...
                console.log(`🛑 Adding end label to string ${stringIndex}, data length: ${currentDataLength}, segment start: ${segmentStart}`);

                // Add label at the END of the current segment
                if (segmentStart !== undefined && this.spectrogramData[stringIndex] && currentDataLength > 0) {
                    ...
                    const endIndex = data.length - 1; // Current end position

                    console.log(`🔍 Checking label: end=${endIndex}, start=${segmentStart}, hasData=${!!data[endIndex]}`);

                    // Only add label if there was actually a segment with data
                    if (endIndex >= segmentStart && data[endIndex]) {
                        const segmentFreq = data[endIndex];

                        console.log(`📍 Adding label at END of segment (index: ${endIndex}), color: ${segmentColor}`);

                        // Add label at the END of the segment
